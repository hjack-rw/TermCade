...
from xiaolin_showdown.logic.flow.wear import hand_over, record_showdown
...
def test_a_committed_wu_wears_by_one():
    ...
    record_showdown(player, [player.hand[0]], rng=Rng(0), wear_limit=WEAR_LIMIT)
...
def test_the_last_showdown_vaults_the_wu_for_its_points():
    ...
    vaulted = record_showdown(player, [worn], rng=Rng(0), wear_limit=WEAR_LIMIT)
...
def test_vaulting_by_wear_spends_no_action():
    ...
    record_showdown(state.player, [worn], rng=Rng(0), wear_limit=WEAR_LIMIT)
...
def test_a_wu_the_showdown_took_away_does_not_wear_for_its_old_owner():
    ...
    player = duelist(hand=[])
    record_showdown(player, [lost_card], rng=Rng(0), wear_limit=WEAR_LIMIT)
...
def test_the_inalienable_wudai_never_wears():
    ...
    record_showdown(player, [dragon], rng=Rng(0), wear_limit=WEAR_LIMIT)
