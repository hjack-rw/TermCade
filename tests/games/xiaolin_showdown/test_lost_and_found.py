...
from factories import ground, plain_wu, plain_wu_agility

PLAIN_WU = plain_wu(load_catalog()).id
ANOTHER_PLAIN_WU = plain_wu_agility(load_catalog()).id
...
def test_the_rooster_calls_back_the_oldest_lost_wu(state, card, settings):
    ...
    first, second = card(PLAIN_WU), card(ANOTHER_PLAIN_WU)
