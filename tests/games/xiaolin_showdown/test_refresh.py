...
def test_refresh_calls_back_the_last_used_wu(state, settings):
    reverso = _reverso()
    spent = wu(1, name="Spent")
    state.player.hand = [reverso]
    state.used = [wu(2, name="Older"), spent]  # most recent last

    use_power(state, reverso, settings, is_player=True)
...
def test_refresh_takes_the_opponents_wu_when_it_was_used_last(state, settings):
    reverso = _reverso()
...
def test_a_refreshed_wu_comes_back_healed(state, settings):
    reverso = _reverso()
    worn = wu(1, name="Worn")
...
def test_refresh_fizzles_with_nothing_used(state, settings):
    reverso = _reverso()
    state.player.hand = [reverso]
    state.used = []

    message = use_power(state, reverso, settings, is_player=True)

    assert "should have happened" in message.toast  # the fizzle wording


def test_a_used_power_lands_in_the_shared_pile(state, settings):
    reverso = _reverso()
    state.player.hand = [reverso]
...
def test_refresh_is_offered_only_with_a_used_wu(state, settings):
    reverso = _reverso()
    state.player.hand = [reverso]

    state.used = []
    assert not is_one_of(reverso, usable_powers(state, DEPOSIT_LIMIT, settings))

    state.used = [wu(1)]
    assert is_one_of(reverso, usable_powers(state, DEPOSIT_LIMIT, settings))
