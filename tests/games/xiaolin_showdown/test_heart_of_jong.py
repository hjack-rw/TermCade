...
from xiaolin_showdown.logic.flow.summons import _JONG_FORMS
...
def test_fielded_as_a_card_it_is_a_middling_metal_body():
    ...
    resolve_played_power(battle, deepcopy(load_catalog().card(ANIMATE)), is_player=True, element="metal")
...
def test_as_a_boost_it_wakes_to_a_flat_form():
    woke = as_boost(deepcopy(load_catalog().card(ANIMATE)), "fire", "force")
...
def test_the_form_always_counts_as_the_background_not_metal():
    ...
    woke = as_boost(deepcopy(load_catalog().card(ANIMATE)), "water", "force")
    assert woke.element == "water"


def test_the_form_is_named_by_the_background():
    duel = _duel_on("water")
    duel._commit_boost(deepcopy(duel.state.catalog.card(ANIMATE)), is_player=True, element="water")
...
def test_boosting_the_heart_flags_the_summoner():
    duel = _duel_on("metal")
    duel._commit_boost(deepcopy(duel.state.catalog.card(ANIMATE)), is_player=True, element="metal")
...
def test_the_heart_is_not_banked_as_junk():
    ...
    assert duel_value(load_catalog().card(ANIMATE)) > 0
